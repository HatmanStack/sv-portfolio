...
### Task 0: Update Dependencies and Fix Security Vulnerabilities

**Goal**: Update all dependencies to their latest compatible versions and resolve security vulnerabilities identified by GitHub/npm audit before starting the refactoring work.
...
1. Check current security vulnerabilities:
   ...
   Document the vulnerabilities found (GitHub reported: 1 high, 9 moderate, 3 low)

2. Review `package.json` dependencies:
   - Note current versions of all dependencies
   - Identify which are outdated
   - Check for major version updates that might break compatibility

3. Update dependencies strategically:
   - **Start with security fixes**: `pnpm audit fix`
   - This will update packages to fix vulnerabilities while respecting semver
   - Review the changes before committing

4. If `pnpm audit fix` doesn't resolve all issues:
   - Manually update specific vulnerable packages
   - Check each package's changelog for breaking changes
   - Update one package at a time for complex updates

5. Update other dependencies (non-security):
   - Consider updating to latest minor/patch versions
   - Check Svelte, SvelteKit, Vite versions
...
7. Fix any breaking changes:
   - If updates break the build, check changelogs
   - Fix compatibility issues
   - May need to adjust code for new API

8. Verify vulnerabilities resolved:
   ```bash
   pnpm audit
   ```
...
9. Document what was updated:
   - List major package updates
   - Note any breaking changes handled
   - Document remaining vulnerabilities (if any)

**Verification Checklist**:
- [ ] `pnpm audit` run and vulnerabilities documented
- [ ] Security vulnerabilities resolved (or documented if can't be fixed)
...
- Audit: Run `pnpm audit` before and after
- Install: Verify `pnpm install` completes
- Type Check: `pnpm run check` should pass
- Dev: Test app in development mode
- Build: Test production build
- Manual: Quick test of all routes

**Commit Message Template**:
```
chore(deps): update dependencies and fix security vulnerabilities

- Run pnpm audit and document vulnerabilities
- Update packages to fix security issues
- Update other dependencies to latest compatible versions
- Test application after updates
- Resolve X high, X moderate, X low severity issues
- Verify build and application functionality
```
