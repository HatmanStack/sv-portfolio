...
export const androidApps: AndroidApp[] = [
	{
		...
		description:
			'Audio looper with record/import, per-track controls (stop/pause/delete), volume/speed sliders, and local file saving.',
		initialImg: loopers,
		activeImg: looper
	},
	{
	...
	},
	{
		...
		description:
			'TMDB client demonstrating Android LiveData and Room. On-device database syncs with web data in real-time.',
		initialImg: moviess,
		activeImg: movies
	},
	{
	...
	},
	{
		...
		description:
			'Restaurant PoC with menu/nutrition info, location search, and custom map markers showing hours and contact details. (Deprecated)',
		initialImg: italians,
		activeImg: italian
	}
];

export const androidContentMap = androidApps.reduce<Record<string, AndroidApp | ContentMapSeed>>(
	(acc, app) => {
		acc[app.title] = app;
		return acc;
	},
